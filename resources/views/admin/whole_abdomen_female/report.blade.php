...
<body>
    ...
    <div class="print_page" id="print_page">
        ...
        <table class="address_table">
            <tr>
                <td>Pt. ID</td>
            <td>{{$wholeAbdomenFemale->patient->id}}</td>
                <td>Date: </td>
                <td>{{ \Carbon\Carbon::parse($wholeAbdomenFemale->created_at)->format('d/m/Y') }}</td>
                <td>Age:</td>
                <td colspan="2">{{$wholeAbdomenFemale->patient->age}} Years</td>
            </tr>
            <tr>
                <td>Pt. Name</td>
                <td colspan="3">{{$wholeAbdomenFemale->patient->name}}</td>
                <td>Gender:</td>
                <td>{{$wholeAbdomenFemale->patient->gender}}</td>

            </tr>
            <tr>
                <td>Ref. By</td>
                <td colspan="5">{{$wholeAbdomenFemale->user->name}}</td>
        ...
        <table class="content_table">
            ...
            <tr>
                ...
                <td>Both Kidneys are normal in size, shape and position. Bipolar measurement of right kidney is about
                    <strong>{{$wholeAbdomenFemale->kidney}} mm.</strong> and that of the left is
                    <strong>{{$wholeAbdomenFemale->kidney_left}} mm.</strong> Parenchymal thickness of RK
                    <strong>{{$wholeAbdomenFemale->rk}} mm.</strong> and that of LK is
                    <strong>{{$wholeAbdomenFemale->lk}} mm.</strong> Cortico-sinusal differentiation is maintained. Pelvicalyceal system of both kidneys are not dilated. Parenchymal thickness and echogenicity appear normal. No Calculus or SOL is detected.
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>:</td>
                <td>{{$wholeAbdomenFemale->interpretation}} Normal findings at USG.</td>
            </tr>
        </table>
